	...
	for i in range(len(documents)):
		...
		if len(document) > 0:
			...
			document = [re.sub(r'\b\'m|\'s|\'re|\'d|\'ll|n\'t\b', '', token) for token in document]
 			# No numbers
			document = [re.sub(r'\b\d+\.?\d*\b', '', token) for token in document]
